(function () {
  ...
  function normalizeJobTotals(summary) {
    var jobs = summary && summary.jobs ? summary.jobs : {};
    return {
      queued: typeof jobs.queued === 'number' ? jobs.queued : 0,
      processing: typeof jobs.processing === 'number' ? jobs.processing : 0,
      partial: typeof jobs.partial === 'number' ? jobs.partial : 0,
      failed: typeof jobs.failed === 'number' ? jobs.failed : 0,
      completed: typeof jobs.completed === 'number' ? jobs.completed : 0
    };
  }

  function normalizeSesAccount(health) {
    var sesAccount = health && health.sesAccount ? health.sesAccount : {};
    var sendQuota = sesAccount.sendQuota || {};

    return {
      available: sesAccount.available === true,
      mode: sesAccount.mode || 'unknown',
      productionAccessEnabled: sesAccount.productionAccessEnabled === true,
      sendingEnabled: sesAccount.sendingEnabled === true,
      enforcementStatus: sesAccount.enforcementStatus || '',
      checkedAt: sesAccount.checkedAt || '',
      error: sesAccount.error || '',
      sendQuota: {
  ...
  function formatSesAccountError(sesAccount) {
    var message = sesAccount && sesAccount.error ? String(sesAccount.error) : '';
    var lowered = message.toLowerCase();

    if (!message) {
      return 'SES account status unavailable.';
    }
    if (lowered.indexOf('accessdenied') !== -1 || lowered.indexOf('not authorized') !== -1 || lowered.indexOf('not authorized to perform') !== -1) {
      return 'SES account status unavailable. Add ses:GetAccount to the bridge IAM policy.';
    }

    return 'SES account status unavailable: ' + message;
  }

  function sesModeLabel(sesAccount) {
    if (!sesAccount || !sesAccount.available) return 'Unknown';
    return sesAccount.productionAccessEnabled ? 'Production' : 'Sandbox';
  }

  function formatQuotaValue(value, suffix) {
    if (value === undefined || value === null || !Number.isFinite(Number(value))) return '-';
    var numeric = Number(value);
    var rounded = String(Math.round(numeric * 100) / 100);
  ...
  function normalizeDeliveryPayload(payload) {
    ...
    var period = getPeriodConfig(sourcePeriod.key || state.period || '30d');
    var timeline = Array.isArray(source.timeline) ? source.timeline : [];

    return {
      sent: toNumber(source.sent),
      delivered: toNumber(source.delivered),
      opened: toNumber(source.opened),
      clicked: toNumber(source.clicked),
      failed: toNumber(source.failed),
      complained: toNumber(source.complained),
      period: {
        key: sourcePeriod.key || period.key,
        buckets: toNumber(sourcePeriod.buckets) || period.buckets,
        stepHours: toNumber(sourcePeriod.stepHours) || period.stepHours,
        label: sourcePeriod.label || period.label,
        axisLabel: sourcePeriod.axisLabel || period.axisLabel
      },
      rates: {
        delivery: toNumber(source.rates && source.rates.delivery),
        open: toNumber(source.rates && source.rates.open),
        click: toNumber(source.rates && source.rates.click),
        failure: toNumber(source.rates && source.rates.failure),
        complaint: toNumber(source.rates && source.rates.complaint)
      },
      timeline: timeline.map(function (row) {
        var hour = new Date(row && row.hour);
        if (!Number.isFinite(hour.getTime())) hour = new Date(0);
        return {
          hour: hour,
          sent: toNumber(row && row.sent),
          delivered: toNumber(row && row.delivered),
          opened: toNumber(row && row.opened),
          clicked: toNumber(row && row.clicked),
          failed: toNumber(row && row.failed),
          complained: toNumber(row && row.complained)
        };
      })
    };
  }
  ...
  function metricSparkline(label, value, seriesValues) {
    var width = 110;
    var gridTop = 8;
    var gridBottom = 42;
    var height = gridBottom;
    var plotTop = 8;
    var plotBottom = 42;
    var values = [];
    var i;
    ...
    if (Array.isArray(seriesValues) && seriesValues.length) {
      values = seriesValues.map(function (entry) {
        return toNumber(entry);
      });
    } else {
      values = [metricNumber(value), metricNumber(value)];
    }
    ...
    var points = values.map(function (pointValue, index) {
      var x = (width * index) / Math.max(values.length - 1, 1);
      var ratio = (axisMax - pointValue) / Math.max(axisMax - axisMin, 0.0001);
      var y = plotTop + (ratio * (plotBottom - plotTop));
    ...
    var deliveryTimeline = state.delivery && Array.isArray(state.delivery.timeline)
      ...
      : [];
    var start = deliveryTimeline[0] ? new Date(deliveryTimeline[0].hour) : null;
    var end = deliveryTimeline.length ? new Date(deliveryTimeline[deliveryTimeline.length - 1].hour) : null;
    ...
    var endLabel = formatSparkLabel(end);

    sparkUid += 1;
    var chartId = 'spark-' + sparkUid.toString(36) + '-' + Math.abs((label + '|' + state.period).split('').reduce(function (acc, ch) {
  ...
  function metric(label, value, suffix, viewTarget, seriesValues) {
    ...
    return '<article class="' + classes.join(' ') + '">' +
      ...
      metricSparkline(label, value, seriesValues) +
      '</article>';
  }

  function insightsChartHeight() {
    var viewport = window.innerWidth || document.documentElement.clientWidth || 1280;
    var vwHeight = viewport * 0.16;
    return Math.max(180, Math.min(320, vwHeight));
  }
  ...
  function deliveryMetricSeries(delivery, metricKey) {
    ...
    return timeline.map(function (row, idx) {
      var sent = toNumber(row.sent);
      var delivered = toNumber(row.delivered);
      var opened = toNumber(row.opened);
      var clicked = toNumber(row.clicked);
      var value = 0;
      ...
      return { idx: idx, value: Math.max(0, Math.min(100, value)) };
    });
  }
  ...
  function failureLineSeries(delivery, metricKey) {
    ...
    return timeline.map(function (row, idx) {
      ...
      if (metricKey === 'failure') {
        numerator = toNumber(row.failed);
        denominator = toNumber(row.sent);
      } else {
        numerator = toNumber(row.complained);
        denominator = toNumber(row.delivered);
      }
  ...
  function suppressionLineSeries(type) {
    ...
    var deliveryTimeline = Array.isArray(delivery.timeline)
      ? delivery.timeline
      : [];
    if (!deliveryTimeline.length) return [];

    var startMs = deliveryTimeline[0].hour.getTime();
    var stepHours = toNumber(delivery.period && delivery.period.stepHours) || 24;
    var bucketMs = Math.max(1, stepHours) * 3600000;
    ...
    state.suppressions.forEach(function (row) {
      ...
      var idx = Math.floor((ts - startMs) / bucketMs);
      if (idx < 0 || idx >= buckets.length) return;
      buckets[idx] += 1;
    });

    return buckets.map(function (count, idx) {
      return { idx: idx, value: count };
    });
  }
  ...
  function renderOverview() {
    ...
    var sesAccount = normalizeSesAccount(health);
    ...
    var jobTotals = normalizeJobTotals(summary);
    var totalSuppressions = suppressionTotals.bounces + suppressionTotals.complaints + suppressionTotals.unsubscribes;
    var sentSeries = Array.isArray(delivery.timeline)
      ? delivery.timeline.map(function (row) { return toNumber(row.sent); })
      : [];
    var failureSeries = failureLineSeries(delivery, 'failure').map(function (point) { return toNumber(point.value); });
    var suppressionSeries = suppressionLineSeries(null).map(function (point) { return toNumber(point.value); });
    var queueSeries = [jobTotals.queued, jobTotals.processing, jobTotals.partial, jobTotals.failed];

    overviewCardsEl.innerHTML = [
      metric('Queued batches', jobTotals.queued, null, 'health', queueSeries),
      metric('Processing', jobTotals.processing, null, 'health', [jobTotals.processing, jobTotals.completed, jobTotals.failed]),
      metric('Sent', summary.sent_24h, null, 'delivery', sentSeries),
      metric('Failure rate', delivery.rates.failure, '%', 'failures', failureSeries),
      metric('Suppressions', totalSuppressions, null, 'suppressions', suppressionSeries)
    ].join('');
    ...
    if (jobTotals.failed > 0) {
      alerts.push({ level: 'danger', text: jobTotals.failed + ' batch' + (jobTotals.failed === 1 ? ' is' : 'es are') + ' currently failed.' });
    }
    if (jobTotals.partial > 0) {
      alerts.push({ level: 'warn', text: jobTotals.partial + ' batch' + (jobTotals.partial === 1 ? ' is' : 'es are') + ' partially sent.' });
    }
    if (jobTotals.queued > 0) {
      alerts.push({ level: 'warn', text: jobTotals.queued + ' batch' + (jobTotals.queued === 1 ? ' is' : 'es are') + ' waiting in the send queue.' });
    }
    if (failureRate > 0.05) {
    ...
    if (health.worker && health.worker.lastErrorMessage) {
      alerts.push({ level: 'danger', text: 'Worker error: ' + health.worker.lastErrorMessage });
    }
    if (health.poller && health.poller.lastErrorMessage) {
      alerts.push({ level: 'danger', text: 'Poller error: ' + health.poller.lastErrorMessage });
    }
    if (sesAccount.available && !sesAccount.productionAccessEnabled) {
      alerts.push({
        level: 'warn',
        text: 'SES sandbox active in ' + String(runtimeConfig.awsRegion || 'this region') + '. Only verified recipients can receive mail.'
      });
    }
    if (sesAccount.available && sesAccount.sendingEnabled === false) {
      alerts.push({ level: 'danger', text: 'SES sending is disabled for this account.' });
    }
    if (!sesAccount.available && sesAccount.error) {
      alerts.push({ level: 'warn', text: formatSesAccountError(sesAccount) });
    }
    if (!alerts.length) {
  ...
  function renderDelivery() {
    ...
    deliveryRowsEl.innerHTML = delivery.timeline.map(function (row) {
      ...
      var sent = row.sent || 0;
      var openedPct = sent ? Math.round((row.opened / sent) * 100) + '%' : '0%';
      var clickedPct = sent ? Math.round((row.clicked / sent) * 100) + '%' : '0%';
      var failedPct = sent ? Math.round((row.failed / sent) * 100) + '%' : '0%';
      return '<tr>' +
        '<td class="mono">' + esc(point) + '</td>' +
        '<td class="mono align-right">' + esc(sent) + '</td>' +
        '<td class="mono align-right"><span class="rate-default">' + openedPct + '</span><span class="rate-hover">' + esc(row.opened) + '</span></td>' +
        '<td class="mono align-right"><span class="rate-default">' + clickedPct + '</span><span class="rate-hover">' + esc(row.clicked) + '</span></td>' +
        '<td class="mono align-right"><span class="rate-default">' + failedPct + '</span><span class="rate-hover">' + esc(row.failed) + '</span></td>' +
        '</tr>';
    }).join('');
  }
  ...
  function buildIntegrationChecks() {
    ...
    var expectedBulkEmailBaseUrl = String(runtimeConfig.expectedBulkEmailBaseUrl || '').trim();
    ...
    return [
      ...
      },
      {
        ...
        status: worker.isRunning ? 'ok' : 'warn',
        detail: worker.isRunning ? 'Send worker loop active' : 'Worker loop is not running'
      },
      {
        name: 'SES event poller',
        status: poller.isRunning ? 'ok' : 'warn',
        detail: poller.isRunning ? 'Event poller loop active' : 'Poller loop is not running'
      },
      {
        name: 'Queued batches',
        status: toNumber(jobs.queued) === 0 ? 'ok' : 'warn',
        detail: String(toNumber(jobs.queued)) + ' batch(es) waiting to send'
      },
      {
        name: 'Failed batches',
        status: toNumber(jobs.failed) === 0 ? 'ok' : 'warn',
        detail: String(toNumber(jobs.failed)) + ' batch(es) failed'
      },
      {
        name: 'Delivery rate',
      ...
      {
        name: 'Recent worker errors',
        status: worker.lastErrorMessage ? 'warn' : 'ok',
        detail: worker.lastErrorMessage || 'No recent worker errors'
      },
      {
        name: 'SES account mode',
        status: !sesAccount.available || !sesAccount.productionAccessEnabled ? 'warn' : 'ok',
        detail: sesAccount.available
          ? sesModeLabel(sesAccount) + ' in ' + String(runtimeConfig.awsRegion || 'configured region')
          : formatSesAccountError(sesAccount)
      },
      {
        name: 'SES sending enabled',
        status: !sesAccount.available || sesAccount.sendingEnabled === false ? 'warn' : 'ok',
        detail: sesAccount.available
          ? (sesAccount.sendingEnabled ? 'SES can accept outbound sends' : 'SES sending is disabled for this account')
          : 'Unavailable until SES account status can be read'
      },
      {
        name: 'Ghost bulk email target',
        status: expectedBulkEmailBaseUrl ? 'ok' : 'warn',
        detail: expectedBulkEmailBaseUrl
          ? 'Ghost should use ' + expectedBulkEmailBaseUrl + '. If sends still time out after a migration, check for an old stored mailgun_base_url.'
          : 'Set Ghost bulkEmail__mailgun__baseUrl to the bridge service URL.'
      }
    ];
  }
  ...
  function renderHealth() {
    ...
    var worker = health.worker || {};
    var jobs = health.jobs || {};
    var queues = health.queues || {};
    var sesAccount = normalizeSesAccount(health);
    var sendQueue = queues.newsletterSend || {};
    var eventQueue = queues.sesEvents || {};
    var statusValue = String(health.status || 'unknown').toLowerCase();
    var statusClass = 'warn';
    var queueDepth = null;
    var sendQueueDepth = null;
    var pollerUpdate = poller.lastPollFinishedAt || poller.lastPollStartedAt || null;
    var workerUpdate = worker.lastPollFinishedAt || worker.lastPollStartedAt || null;
    ...
    if (typeof health.sendQueueDepth === 'number') {
      sendQueueDepth = health.sendQueueDepth;
    } else if (typeof sendQueue.visible === 'number') {
      sendQueueDepth = toNumber(sendQueue.visible) + toNumber(sendQueue.inFlight) + toNumber(sendQueue.delayed);
    }
    ...
    var rows = [
      ['SES account mode', sesModeLabel(sesAccount)],
      ['SES sending enabled', !sesAccount.available ? '-' : (sesAccount.sendingEnabled ? 'Yes' : 'No')],
      ['SES enforcement status', sesAccount.enforcementStatus || '-'],
      ['SES max send rate', formatQuotaValue(sesAccount.sendQuota.maxSendRate, '/sec')],
      ['SES 24h quota', formatQuotaValue(sesAccount.sendQuota.max24HourSend, '')],
      ['Newsletter send queue', sendQueueDepth === null ? '-' : String(sendQueueDepth)],
      ['SES event queue', queueDepth === null ? '-' : String(queueDepth)],
      ['Queued batches', String(toNumber(jobs.queued))],
      ['Processing batches', String(toNumber(jobs.processing))],
      ['Failed batches', String(toNumber(jobs.failed))],
      ['SES account check', sesAccount.checkedAt ? fmtIso(sesAccount.checkedAt) : '-'],
      ['Last worker update', workerUpdate ? fmtIso(workerUpdate) : '-'],
      ['Last poll update', pollerUpdate ? fmtIso(pollerUpdate) : '-'],
      ['Last worker error', worker.lastErrorMessage || 'None'],
      ['Last poller error', poller.lastErrorMessage || 'None'],
      ['SES account status error', sesAccount.error || 'None']
    ];
  ...
  async function load() {
    ...
    try {
      ...
      var responses = await Promise.all([
        ...
        fetch(state.basePath + '/api/delivery?period=' + encodeURIComponent(state.period)),
      ...
      state.delivery = normalizeDeliveryPayload(await responses[1].json());
      state.health = await responses[2].json();

      var failurePayload = await responses[3].json();
      state.failures = failurePayload.items || [];

      var suppressionPayload = await responses[4].json();
      state.suppressionTotals = normalizeSuppressionTotals(state.summary);
