(function () {
  ...
  function metricSparkline(label, value) {
    ...
    return '<div class="metric-spark-wrap">' +
      ...
      '<defs>' +
      '<linearGradient id="' + chartId + '-area" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="0" y2="32">' +
    ...
    var metrics = [
      {
        ...
        displayType: 'count',
        detail: 'Delivered ' + formatCount(delivery.delivered)
      },
  ...
  function deliveryInsightsChart(delivery, activeMetric) {
    var width = 760;
    var height = 236;
    var chartLeft = 46;
    var chartRight = 16;
    ...
    if (activeMetric.key === 'delivery') {
      ...
      return '<div class="delivery-insights-chart-wrap">' +
        ...
        '<text class="delivery-insights-x-label" x="' + chartLeft + '" y="' + (height - 6) + '" text-anchor="start">' + esc(firstLabel) + '</text>' +
        '<text class="delivery-insights-x-label" x="' + (chartLeft + plotWidth) + '" y="' + (height - 6) + '" text-anchor="end">' + esc(lastLabel) + '</text>' +
        '</svg>' +
        '</div>';
    }
  ...
  function renderDeliveryInsights(delivery) {
    ...
    return '<div class="delivery-insights-shell" style="--delivery-chart-color: ' + tabs.active.colorVar + ';">' +
      ...
      '</div>' +
      deliveryInsightsChart(delivery, tabs.active) +
      '</div>';
  }
  ...
  function renderLineInsightsChart(series, options) {
    ...
    return '<div class="delivery-insights-chart-wrap">' +
      ...
      '<path class="delivery-insights-area" fill="url(#' + gradientId + ')" d="' + areaPath + '"></path>' +
      '<polyline class="delivery-insights-line" points="' + linePoints.join(' ') + '"></polyline>' +
      '<text class="delivery-insights-x-label" x="' + chartLeft + '" y="' + (height - 6) + '" text-anchor="start">' + esc(labels.first) + '</text>' +
      '<text class="delivery-insights-x-label" x="' + (chartLeft + plotWidth) + '" y="' + (height - 6) + '" text-anchor="end">' + esc(labels.last) + '</text>' +
      '</svg>' +
      '</div>';
  }
