(function () {
  ...
  function renderHealth() {
    ...
    if (sesAccount.available) {
      rows.splice(1, 0, ['SES sending enabled', sesAccount.sendingEnabled ? 'Yes' : 'No']);
      rows.splice(2, 0, ['SES enforcement status', sesAccount.enforcementStatus || '-']);
      rows.push(['SES account check', sesAccount.checkedAt ? fmtIso(sesAccount.checkedAt) : '-']);
    }

    if (worker.lastErrorMessage) {
      rows.push(['Last worker error', worker.lastErrorMessage]);
    }

    if (poller.lastErrorMessage) {
      rows.push(['Last poller error', poller.lastErrorMessage]);
    }

    if (sesAccount.error) {
      rows.push(['SES account status error', sesAccount.error]);
    }
