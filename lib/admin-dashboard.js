...
var labPublicDir = path.join(__dirname, '..', 'dashboard-lab', 'public');
var labIndexPath = path.join(labPublicDir, 'index.html');
var labStylesPath = path.join(labPublicDir, 'styles.css');
var labAppPath = path.join(labPublicDir, 'app.js');
...
function createAdminRouter(getPollerState) {
  ...
  router.get('/styles.css', function(_req, res) {
    try {
      res.type('css').send(readLabAsset(labStylesPath));
    } catch (err) {
  ...
  router.get('/app.js', function(_req, res) {
    try {
      res.type('application/javascript').send(readLabAsset(labAppPath));
    } catch (err) {
