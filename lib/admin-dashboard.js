...
var countUniqueEventTypeSince = db.prepare(
  'SELECT COUNT(*) as c FROM (' +
  ' SELECT 1 FROM events' +
  ' WHERE event_type = ? AND timestamp >= ?' +
  " GROUP BY lower(recipient), COALESCE(NULLIF(message_id, ''), NULLIF(email_id, ''), '')" +
  ')'
);
...
function buildGhostSessionVerifyUrl() {
  ...
  if (!base) return null;
  // Use /users/me/ which requires a valid admin session.
  // The /site/ endpoint is PUBLIC and returns 200 without auth,
  // so it must NOT be used for session verification.
...
function getSummary() {
  ...
  return {
    ...
    opened_24h: countUniqueEventTypeSince.get('opened', since24h).c,
    clicked_24h: countUniqueEventTypeSince.get('clicked', since24h).c,
