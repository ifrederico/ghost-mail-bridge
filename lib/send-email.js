...
var { insertMessageMap, insertRecipientEmail, insertEvent } = require('./db');
...
function recordImmediateSendFailure(opts) {
...
function handleSendEmail(req, res) {
  parseFormData(req).then(function(fields) {
    ...
    insertMessageMap.run(batchMessageId, emailId, tagsJson);
    ...
    var promises = toList.map(function(recipient) {
      return semaphore.acquire().then(function() {
        return Promise.resolve().then(function() {
          ...
          return sendRawEmailWithRetry(rawMessage, recipient).then(function(outcome) {
          ...
          try {
            ...
            console.error('Failed to record send failure event for ' + recipient + ': ' + (recordErr.message || String(recordErr)));
          }
        }).finally(function() {
          semaphore.release();
        });
